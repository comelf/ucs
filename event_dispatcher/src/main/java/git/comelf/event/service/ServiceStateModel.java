...
public class ServiceStateModel {
     ...
     * @throws RuntimeException if the service state is different from
     *                               the desired state
     */
    public void ensureCurrentState(Service.STATE expectedState) {
        if (state != expectedState) {
            throw new RuntimeException(name + ": for this operation, the " +
    ...
    /**
     * Check that a state transition is valid and
    ...
    public static void checkStateTransition(String name,
        ...
        if (!isValidStateTransition(state, proposed)) {
            throw new RuntimeException(name + " cannot enter state "
